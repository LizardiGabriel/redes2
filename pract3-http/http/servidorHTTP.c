...
void *connection_handler(void *socket_desc) {
    ...
    if (thread_count > 10) {
        // If there is 10 request at the same time, other request will be refused.
    ...
    if (request < 0) {
        ...
        // receive socket closed. Client disconnected upexpectedly.
        ...
        if (strncmp(request_lines[0], "GET\0", 4) == 0) {
            ...
            if (strncmp(request_lines[2], "HTTP/1.0", 8) != 0 && strncmp(request_lines[2], "HTTP/1.1", 8) != 0) {
                // Bad request if not HTTP 1.0 or 1.1
                ...
                /***************************************/
                // For parsing the file name and extension.
                char *tokens[2];
                ...
                if (p1 == NULL) {
                    ...
                    if (tokens[0] == NULL || tokens[1] == NULL) {
                        // If there is not an extension in request or request to just localhost:8888/
                ...
                if (strcmp(tokens[1], "html") != 0 && strcmp(tokens[1], "htm") != 0 && strcmp(tokens[1], "jpeg") != 0 && strcmp(tokens[1], "jpg") != 0 && strcmp(tokens[1], "ico") != 0) // If the request is not to html or jpeg files, it will be respond 400 Bad Request
                {
                    ...
                    if (strcmp(tokens[1], "html") == 0 || strcmp(tokens[1], "htm") == 0) {
                        ...
                        // Prevent two or more thread do some IO operation same time.
                        sem_wait(&mutex);
